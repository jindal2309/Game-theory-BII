...
def comp_cost(x, comp_id, comp_len, Csd, Cinf):
    ...
    for edge in x:
        if x[edge] == 1: 
            ...
            cost[edge[1]] = cost.get(edge[1],0) + Csd[edge]
...
def reduction_in_cost(G, x, comp_id, comp_len, cost, Csd, Cinf, u, edge_list):
    ...
    if x[edge_list[0]] == 0:
        ...
        for edge in edge_list:
            cost_increment += Csd[edge]
            nbr_comp[comp_id[edge[1]]] = 1
        
        z = 0
        for j in nbr_comp: 
            z += comp_len[j]
        cost_increment -= z*Cinf[u]/(len(x)+0.0)
        return -1*cost_increment
...
def update_strategy(x, G, H, comp_d, comp_id, comp_len, cost, Csd, Cinf, comp_max_id, u, edge_list):
...
#             if (itrn % 10 == 0): print(itrn)
            ...
            for v in G.neighbors():
                ...
                edge_group[neighbour_comp_id].append((u,v))

            for neighbour_comp_id, edge_list in edge_group.items():
                if reduction_in_cost(G, x, comp_id, comp_len, cost, Csd, Cinf, u, edge_list) > 0:
                    x, comp_d, comp_id, comp_len, cost, comp_max_id = update_strategy(x, 
                                        G, H, comp_d, comp_id, comp_len, cost, Csd, Cinf, comp_max_id, u, edge_list)
